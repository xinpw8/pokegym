...
def relocate(game, y, x):
    r_pos = write_mem(game, Y_POS_ADDR, y)
    c_pos = write_mem(game, X_POS_ADDR, x)
    r, c, n = position(game)
...
def award_badge(game, addr, badge_number):
    """ Award a badge by setting the appropriate bit in memory. """
    current_badges = game.get_memory_value(addr)
    new_badges = set_bit(current_badges, badge_number - 1)  # Subtract 1 because bits are 0-indexed
    write_mem(game, addr, new_badges)

def remove_badge(game, addr, badge_number):
    """ Remove a badge by clearing the appropriate bit in memory. """
    current_badges = game.get_memory_value(addr)
    new_badges = clear_bit(current_badges, badge_number - 1)
    write_mem(game, addr, new_badges)
