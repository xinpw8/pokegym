...
# BET ADDED - not used in final version of boey code
...
class Environment(Env):
    # Order and name envs with multiprocessing
    counter_lock = multiprocessing.Lock()
    counter = multiprocessing.Value('i',0)
    shared_length = multiprocessing.Value('i',0)
    lock = multiprocessing.Lock()
    
    def __init__(self, env_config=None, **kwargs):
        # Name the env folders sequentially and atomically
        ...
        # Directory for required resources relative to the found base_dir
        required_resources_dir = start_dir / "required_resources"
        # Ensure the required_resources directory exists
        required_resources_dir.mkdir(parents=True, exist_ok=True)
        # Files to be created in the required_resources directory
        files = ["running_experiment.txt", "test_exp.txt"]
        # Create the files if they do not exist
        for file_name in files:
            file_path = required_resources_dir / file_name
        ...
        with open(start_dir / "required_resources/running_experiment.txt", "r") as file:
            exp_name = file.read()
        ...
        self.max_steps = env_config['max_steps'] # env_config['max_steps']
...
Dict('event_ids': Box(0, 2570, (128,), int16), 'event_step_since': Box(-1.0, 1.0, (128, 1), float32), 
    ...
    def step(self, action):
        # print(f'self.step_count >= self.max_steps: {self.step_count} >= {self.max_steps}')
        ...
        #         step_limit_reached = True
        #         print(f'BEATEN CHAMPION at step {self.env_id}:{self.step_count}')
