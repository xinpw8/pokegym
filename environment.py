...
cpu_multiplier = 2.0  # 1.0 = 32 num_cpu, this is to scale and maintain the same number of training steps per training iteration
...
config = {
                ...
                'action_freq': 24, 'init_state': state_path, 'max_steps': ep_length, 
                'print_rewards': False, 'save_video': False, 'fast_video': True, 'session_path': sess_path,
                'gb_path': 'PokemonRed.gb', 'debug': False, 'sim_frame_dist': 2_000_000.0, 
                'use_screen_explore': True, 'reward_scale': 4, 
...
class Environment(Env):
    ...
    def __init__(
        ...
        self.use_screen_explore = False if 'use_screen_explore' not in config else config['use_screen_explore'] # True
    ...
    def step(self, action):
        ...
        #             'opponent_level': self.progress_reward['op_lvl'],
        #             'death': self.progress_reward['dead'],
    ...
    def save_and_print_info(self, done, obs_memory):
        ...
        if self.step_count % 10000 == 0: # 1000
            try:
                plt.imsave(
